...
const BuyCourseItem = ({pic , name , teacher , term , startDate , cost}) =>{
  return(
...
const BuyCourse = () => {
  ...
  const [buyCourses , setBuyCourses] = useState([
    ...
    {pic:'c4.png' , name : 'دوره ریکت' , teacher : 'دکنر بحرالعلوم' , term : 'بهار ' , startDate : '1400/02/02' , cost : 250000   },
  ])
  ...
  const smallList = buyCourses.filter((item , index)=>index >= start && index <= end)

  
  return (
    <div className='xl:flex mx-auto flex-wrap justify-center my-8 lg:pt-6 text-center font-irSans'>
      <h2 className='block  my-12 lg:my-0  lg:w-full lg:text-center text-2xl font-extrabold'>لیست دوره ها  </h2>
      ...
      <div className='w-10/12 mx-auto lg:w-8/12 xl:w-6/12 lg:mx-auto lg:mt-8'>
          ...
          <div className='hidden rounded-t-xl lg:flex pr-4 py-2 text-md text-white bg-pannel '>
            <h3 className='pr-3 xl:pr-2 2xl:pr-8'>نصویز</h3>
            <h3 className='pr-10 xl:pr-8 2xl:pr-14'>نام دوره</h3>
            <h3 className='pr-16 xl:pr-14 2xl:pr-14'>مدرس</h3>
            <h3 className='pr-16 xl:pr-18 2xl:pr-22'>قیمت </h3>
          </div>

          {smallList.map((item , index)=>{
            return(
              <BuyCourseItem key={index} pic={item.pic} name={item.name} teacher={item.teacher} 
              term={item.term} startDate={item.startDate} cost={item.cost}/>            
            )
          })}

          <div className='mt-8'>
            <Pagination total={buyCourses.length} pageSize={6} showQuickJumper onChange={pageSize=>{changeStart(pageSize)}}/>
          </div>
      ...
      <div className='xl:flex text-center flex-wrap mt-8 p-4 lg:w-8/12 xl:w-3/12 mx-auto bg-mygray'>
        ...
        <div className='my-8 xl:my-4 w-2/3 mx-auto'>
          <label className='block text-center ' htmlFor='fname'>کد تخفیف</label>
        ...
        <Button title='ادامه خرید' backgroundColor='bg-green-400' borderWidth='border-none' extra='h-10 mx-auto'></Button>  
